...
class Pieza:
    ...
    def colision(self,tablero):
        for i in range(len(self.forma)):
            for j in range(len(self.forma[i])):
                if self.forma[i][j] == "X":
                    if self.y + i*tamano_bloque >= alto_pantalla or self.x + j*tamano_bloque < 0 or self.x + j*tamano_bloque>= ancho_pantalla:
                        return True
        return False
...
while not game_over:
            ...
            for i in range(len(pieza_actual.forma)):
                for j in range(len(pieza_actual.forma[i])):
                    if pieza_actual.forma[i][j] == "X":
                        if 0 <= pieza_actual.y + i < len(tablero) and 0 <= pieza_actual.x + j < len(tablero[0]):
    ...
    for i in range(len(tablero)):
        for j in range(len(tablero[i])):
            if tablero[i][j] != ".":
                color_index = colores.index(tablero[i][j])
                pygame.draw.rect(pantalla, colores[color_index], (j, i * tamano_bloque, tamano_bloque, tamano_bloque))
                pygame.draw.rect(pantalla, NEGRO, (j, i * tamano_bloque, tamano_bloque, tamano_bloque), 1)
